...
public class Parser {
	...
	public static Scene createScene(Node dataNode) {
        ...
        if(blurAttr != null){
            Scanner s = new Scanner(blurAttr.getNodeValue());
            scene.blur = s.nextBoolean();
            s.close();

            Node blurSampleAttr = dataNode.getAttributes().getNamedItem("blursample");
            s = new Scanner(blurSampleAttr.getNodeValue());
            scene.blur_sample = s.nextInt();
            s.close();

            Node blurFocalLength = dataNode.getAttributes().getNamedItem("focal");
            s = new Scanner(blurFocalLength.getNodeValue());
            scene.focallength = s.nextDouble();
            s.close();
        }

        Node motionAttr = dataNode.getAttributes().getNamedItem("motion");
        if(motionAttr != null){
            Scanner s = new Scanner(motionAttr.getNodeValue());
            scene.motion = s.nextBoolean();
            s.close();

            Node motionSpeedAttr = dataNode.getAttributes().getNamedItem("speed");
            s = new Scanner(motionSpeedAttr.getNodeValue());
            scene.speed = s.nextDouble();
            s.close();
        }
	...
	public static Render createRender(Node dataNode) {
    	...
    	// Parse the jitter attribute
		Node jitter = dataNode.getAttributes().getNamedItem("jitter");
    	if(jitter != null){
			if("true".equals(jitter.getNodeValue())) {
				render.jitter = true;
			}
		}
